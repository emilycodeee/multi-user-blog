...
export default {
  ...
  setup () {
    ...
    const handleSignin = () => {
      const alertMsg = email.value === '' ? '請填寫 Email' : password.value === '' ? '請填寫密碼' : null
      ...
      logIn(email.value, password.value).then(msg => {
        if (msg) {
          errorMsg.value = msg
        } else {
    ...
    return { email, password, emailIcon, passwordIcon, errorMsg, handleSignin }
  }
}
</script>
